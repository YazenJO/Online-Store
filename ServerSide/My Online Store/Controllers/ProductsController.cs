...
using System.Data;
using ProductImage = OnlineStore.BLL.Image;
    ...
    public class ProductsController : ControllerBase
    {
        ...
        public ActionResult<IEnumerable<object>> GetAllProducts()
        {
            ...
            foreach (DataRow row in productsList.Rows)
            {
                ...
                // Get product images
                var images = ProductImage.GetImagesByProductId(productId);
                ...
                if (images != null && images.Rows.Count > 0)
                {
                    foreach (DataRow imgRow in images.Rows)
                    {
                        imagesList.Add(new
                        {
                            ...
                            imageOrder = (short)imgRow["ImageOrder"]  // Fixed: Changed from (int) to (short)
                        });
                    }
                }
        ...
        public ActionResult<object> GetProductById(int id)
        {
            ...
            // Get product images
            var images = ProductImage.GetImagesByProductId(id);
            var imagesList = new List<object>();
